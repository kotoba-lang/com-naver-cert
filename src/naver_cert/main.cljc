(ns naver_cert.main
  "Kotodama WASM entrypoint for the Naver_cert clean-room actor (L4) — Clojure port.
  ...
  facts (`naver_cert.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "naver_cert")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"naver_cert.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "naver_cert-compat" :tier tier :entities entities} 200])
...
(defn start! [] :naver_cert-compat/ready)
